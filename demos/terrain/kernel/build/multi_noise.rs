...
pub(crate) fn build() -> TokenStream {
    ...
    stream.extend(quote! {
        ...
        use steel_utils::climate::{Parameter, ParameterPoint, FlatNode, find_static};
    });
    ...
    for (preset_name, entries) in &presets {
        ...
        // The climate index is seed-independent metadata. Build it once in the
        // compiler, preserving the reference tree's exact node and tie order.
        let points:Vec<_>=entries.iter().enumerate().map(|(i,e)|{
            use crate::climate::{Parameter,ParameterPoint};
            let p=&e.parameters;let pair=|v:[f64;2]|Parameter::new(quantize(v[0]),quantize(v[1]));
            (ParameterPoint::new(pair(p.temperature),pair(p.humidity),pair(p.continentalness),pair(p.erosion),pair(p.depth),pair(p.weirdness),quantize(p.offset)),i)
        }).collect();
        let tree=crate::climate::ParameterList::new(points);
        let nodes:Vec<_>=tree.nodes().iter().map(|n|{
            let mins=n.mins;let maxs=n.maxs;let value=n.value_index;let start=n.children_start;let count=n.children_count;
            quote!{FlatNode{mins:[#(#mins),*],maxs:[#(#maxs),*],value_index:#value,children_start:#start,children_count:#count}}
        }).collect();
        ...
        stream.extend(quote! {
            ...
            #[doc = #doc_static]
            pub static #static_ident: &[FlatNode] = &[#(#nodes,)*];

            #[doc = #doc_get]
            #[inline]
            pub fn #get_fn(target: &steel_utils::climate::TargetPoint) -> BiomeRef {
                #lookup_fn(find_static(#static_ident,#points_ident,target,&mut None))
            }

            #[doc = #doc_cached]
            #[inline]
            pub fn #get_cached_fn(target: &steel_utils::climate::TargetPoint, cache: &mut Option<usize>) -> BiomeRef {
                #lookup_fn(find_static(#static_ident,#points_ident,target,cache))
            }
        });
    }
