...
pub struct FlatNode {
    ...
    pub mins: [i64; PARAMETER_COUNT],
    /// Bounding box maximum values for each parameter dimension.
    pub maxs: [i64; PARAMETER_COUNT],
...
pub fn find_static(nodes:&[FlatNode],points:&[ParameterPoint],target:&TargetPoint,cache:&mut Option<usize>)->usize {
    let root=&nodes[0];
    if root.is_leaf(){let index=root.value_index as usize;*cache=Some(index);return index;}
    let mut best_idx=cache.map(|i|i as u32);
    let mut best_dist=cache.map_or(i64::MAX,|i|points[i].fitness(target));
    search_nearest(nodes,root,&target.to_parameter_array(),&mut best_dist,&mut best_idx);
    let index=best_idx.expect("nonempty static climate tree") as usize;*cache=Some(index);index
}
...
impl<T> ParameterList<T> {
    /// Build-time serialization uses the exact same tree and stable tie order.
    pub fn nodes(&self)->&[FlatNode]{&self.nodes}
