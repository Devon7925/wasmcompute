...
/// Owned interpolation buffers survive yields without recomputing the column.
pub struct InterpolationColumn { scratch:Vec<f64>,local_x:usize,cell_z_idx:usize }

impl<N: DimensionNoises> NoiseChunk<N> {
    /// Cache Y/X interpolation once for the four adjacent Z columns. The
    /// arithmetic and lane order are identical to fill_partial_detail.
    pub fn prepare_block_column(&self,index:usize)->InterpolationColumn {
        let cell_width=N::Settings::CELL_WIDTH;let cell_height=N::Settings::CELL_HEIGHT;
        let cell_count_xz=self.cell_count_xz;let cell_count_y=self.cell_count_y;
        let corners_y=self.corners_y;let interp_count=self.interp_count;
        assert!(index < cell_count_xz*cell_count_xz*cell_width as usize);
        let cell_x_idx=index/(cell_count_xz*cell_width as usize);
        let cell_z_idx=(index/cell_width as usize)%cell_count_xz;
        let x_in_cell=(index%cell_width as usize) as i32;
        let local_x=(cell_x_idx as i32*cell_width+x_in_cell) as usize;
        let s0=&self.slices[cell_x_idx][..];let s1=&self.slices[cell_x_idx+1][..];
        let z0_base=cell_z_idx*corners_y;let z1_base=(cell_z_idx+1)*corners_y;
        ...
        let mut scratch=vec![0.;column_len*interp_count*2];
        ...
        InterpolationColumn{scratch,local_x,cell_z_idx}
    }

    /// Resume a descending vertical range in one Z column. The caller keeps
    /// the same column order as the full scan, including aquifer cache updates.
    pub fn fill_prepared<F>(&self,noises:&N,cache:&mut N::ColumnCache,
        column:&InterpolationColumn,z_in_cell:i32,rows:std::ops::Range<usize>,mut place_block:F)
    where F:FnMut(usize,i32,usize,f64,&[f64],&mut N::ColumnCache) {
        let cell_width=N::Settings::CELL_WIDTH;let cell_height=N::Settings::CELL_HEIGHT;
        let interp_count=self.interp_count;
        let column_len=self.cell_count_y*cell_height as usize;
        assert!(rows.start<=rows.end && rows.end<=column_len && (0..cell_width).contains(&z_in_cell));
        let (d0_col,d1_col)=column.scratch.split_at(column_len*interp_count);
        let local_x=column.local_x;
        let local_z=(column.cell_z_idx as i32*cell_width+z_in_cell) as usize;
        let factor_z=f64::from(z_in_cell)/f64::from(cell_width);
        let factor_z_v=f64x4::splat(factor_z);
        let mut interpolated=[0.;MAX_INTERP];
        for row in rows.rev() {
            let world_y=self.cell_min_y*cell_height+row as i32;
            let row_base = row * interp_count;
            ...
            // Structures are explicitly excluded from this demo.
    ...
    fn fill_slice_into(
        ...
        for cz in z_range {
            ...
            noises.compute_noise_column(block_x, &block_ys[y_corners.clone()], block_z, &mut blended_column[y_corners.clone()]);

            // 4-Y SIMD-batched corner fill. Tail is handled by the scalar
            // loop below for any remaining `corners_y % 4` corners.
            let mut cy = y_corners.start;
            while cy + 4 <= y_corners.end {
    ...
    {
        self.fill_partial(noises, cache, 0..(self.cell_count_xz+1).pow(2), 0..self.cell_count_xz.pow(2)*N::Settings::CELL_WIDTH as usize, place_block);
    }

    /// Evaluate a bounded subset while preserving the full fill's arithmetic/order.
    pub fn fill_partial<F>(&mut self, noises:&N, cache:&mut N::ColumnCache,
        slice_columns:std::ops::Range<usize>, block_columns:std::ops::Range<usize>, mut place_block:F)
    where F:FnMut(usize,i32,usize,f64,&[f64],&mut N::ColumnCache) {
        self.fill_partial_detail(noises,cache,slice_columns,0..self.corners_y,block_columns,0..N::Settings::CELL_WIDTH,place_block);
    }

    /// The corner boundaries are multiples of four so SIMD lane grouping and
    /// floating-point operand order remain identical to the full evaluation.
    pub fn fill_partial_detail<F>(&mut self,noises:&N,cache:&mut N::ColumnCache,
        slice_columns:std::ops::Range<usize>,y_corners:std::ops::Range<usize>,
        block_columns:std::ops::Range<usize>,block_z:std::ops::Range<i32>,mut place_block:F)
    where F:FnMut(usize,i32,usize,f64,&[f64],&mut N::ColumnCache) {
        let cell_width = N::Settings::CELL_WIDTH;
        let cell_height = N::Settings::CELL_HEIGHT;
        ...
        for index in slice_columns {
            let cx_off=index/n_slices;let cz=index%n_slices;
        ...
        for cell_x_idx in 0..cell_count_xz {
            ...
            for cell_z_idx in 0..cell_count_xz {
                ...
                for x_in_cell in 0..cell_width {
                    let index=(cell_x_idx*cell_count_xz+cell_z_idx)*cell_width as usize+x_in_cell as usize;
                    if !block_columns.contains(&index){continue;}
                    let factor_x = f64::from(x_in_cell) / f64::from(cell_width);
                    let local_x = (cell_x_idx as i32 * cell_width + x_in_cell) as usize;
                    let factor_x_v = f64x4::splat(factor_x);
                    ...
                    // exactly.
                    for z_in_cell in block_z.clone() {
                        let factor_z = f64::from(z_in_cell) / f64::from(cell_width);
