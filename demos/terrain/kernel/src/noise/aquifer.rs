...
pub struct Aquifer<N: DimensionNoises> {
    ...
    /// Resumable constructor scan; visits the same quart columns in z/x order.
    scan_z: i32,
    max_surface: i32,
}
...
impl<N: DimensionNoises> Aquifer<N> {
    ...
    pub fn new_sized(
        ...
        let mut result=Self::new_sized_unprimed(chunk_min_x,chunk_min_z,width_x,width_z,min_block_y,y_block_size,splitter,cache);
        while !result.prime_next(noises) {}
        result
    }

    /// Allocate the aquifer; call `prime_next` to completion before sampling.
    pub fn new_sized_unprimed(
        chunk_min_x:i32,chunk_min_z:i32,width_x:i32,width_z:i32,
        min_block_y:i32,y_block_size:i32,splitter:&RandomSplitter,
        cache:N::ColumnCache,
    ) -> Self {
        ...
        let prelim_cache = FxHashMap::default();
        let skip_sampling_above_y = i32::MAX;
        ...
        Self {
            ...
            should_schedule_fluid_update: false,
            prelim_cache,
            scan_z: from_grid_z(min_grid_z, 0),
            max_surface: i32::MIN,
        }
    }

    /// Scan one quart row. Returns true when the original constructor is done.
    pub fn prime_next(&mut self,noises:&N)->bool {
        let max_z=from_grid_z(self.min_grid_z+self.grid_size_z-1,Z_RANGE-1);
        if !N::Settings::AQUIFERS_ENABLED || self.scan_z>max_z {return true;}
        let max_x=from_grid_x(self.min_grid_x+self.grid_size_x-1,X_RANGE-1);
        let mut x=from_grid_x(self.min_grid_x,0);
        while x<=max_x {
            let level=cached_preliminary_surface_level(noises,&mut self.cache,&mut self.prelim_cache,x,self.scan_z);
            self.max_surface=self.max_surface.max(level);x+=4;
        }
        self.scan_z+=4;
        if self.scan_z>max_z {
            let skip_grid_y=grid_y(self.max_surface+8+12)+1;
            self.skip_sampling_above_y=from_grid_y(skip_grid_y,Y_RANGE+2)-1;
            true
        }else{false}
    }
